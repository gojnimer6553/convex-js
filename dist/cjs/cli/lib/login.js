...
`);
    try {
      ...
      (0, import_log.logError)(import_chalk.chalkStderr.red(`Unable to open browser.`));
      (0, import_log.changeSpinner)(`Manually open ${urlToOpen} in your browser to log in.`);
    }
  } else {
...
async function performPasswordAuthentication(ctx, clientId, username, password) {
  ...
  try {
    ...
    if (err.response) {
      (0, import_log.logError)(import_chalk.chalkStderr.red(`${JSON.stringify(err.response.data)}`));
    }
    return await ctx.crash({
      exitCode: 1,
      errorType: "fatal",
...
async function performLogin(ctx, {
  ...
  } else if (loginFlow === "paste" || loginFlow === "auto" && (0, import_utils.isWebContainer)()) {
  ...
  const response = await (0, import_utils.typedPlatformClient)(ctx).POST(
    "/create_personal_access_token",
    {
      ...
      body: { name: deviceName, anonymousId }
    }
  );
  ...
  await (0, import_deploymentSelection.updateBigBrainAuthAfterLogin)(ctx, newAccessToken);
